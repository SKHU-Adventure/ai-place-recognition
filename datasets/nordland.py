...
class Nordland(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        neg_path = self.get_negative(anc_path,0)
        ...
        if self.loss=="quadruplet":
            another_neg_path = self.get_negative(anc_path, neg_path)
    ...
    def get_negative(self, path, neg_path):
        season_src = path.split("/")[-3]
        section_src = path.split("/")[-2]
        number_src = int(os.path.splitext(path.split("/")[-1])[0])
        ext = os.path.splitext(path.split("/")[-1])[1]
        ...
        if neg_path!=0:
            first_neg_season = neg_path.split("/")[-3]
            first_neg_section = neg_path.split("/")[-2]
            first_neg_number = int(os.path.splitext(neg_path.split("/")[-1])[0])
        while True:
            season_dst = random.choice(self.seasons)[1]
            ...
            if neg_path==0:
                ...
                if ((season_src != season_dst or section_src != section_dst or abs(number_src - number_dst) > self.window) and
                    (first_neg_season != season_dst or first_neg_section != section_dst or abs(first_neg_number-number_dst)>self.window)):
                    break

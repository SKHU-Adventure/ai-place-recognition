...
from utils.util_model import EmbedNet, TripletNet, QuadrupletNet
...
def main():
    ...
    if config.loss=="triplet":
        triplet_net= DDP(TripletNet(embed_net).to(device_id), devices_ids=[device_id])
        ...
        if config.loss=="triplet":
            ...
            with torch.no_grad():
                for i, (anc, pos, neg1, neg2) in enumerate(test_loader):
                    ...
                    losses.update(loss.item(), anc.size(0))
                    dist_poses.update(np.mean(dist_pos), anc.size(0))
                    dist_neges.update(np.mean(dist_neg1), anc.size(0))
    ...
    for epoch in range(1, config.total_epoch + 1):
        ...
        if rank == 0:
            ...
            if config.loss=="triplet":
                torch.save(triplet_net.state_dict(), os.path.join(PATH.CHECKPOINT, f'{config.backbone}_{config.model}_{config.loss}_{config.data}_checkpoint_e{epoch}.pth'))
            else:
                torch.save(quadruplet_net.state_dict(), os.path.join(PATH.CHECKPOINT, f'{config.backbone}_{config.model}_{config.loss}_{config.data}_checkpoint_e{epoch}.pth'))
